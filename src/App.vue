...
<template>
    <section class="bg-white h-screen dark:bg-zinc-900 bg-zinc-900" v-show="state=='welcome'">
        ...
        <div class="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16">
            ...
            <code class="mt-10 p-5 text-sm border text-white dark:text-white bg-zinc-800 border-zinc-700">screen ./navcoind --blsctregtest --printtoconsole --walletcrosschain -rpcuser=<span class="text-blue-500"><code>username</code></span> -rpcpassword=<span class="text-blue-500"><code>password</code></span></code>
...
<div class="relative h-32 w-32">
    <div class="flex items-center p-2 mt-3 space-x-4 justify-self-end">
      ...
      <div>
        <h2 class="text-lg dark:text-white text-white" v-if="client">{{$store.state.active_wallet}}</h2>
  ...
  methods: {
    ...
    connect: function() {
        if(this.username&&this.password)
        {
            ...
            this.$store.commit('set_active_wallet',this.client.wallet);
            const batch = [{ method: "getwalletinfo" }];
            this.client.command(batch).then((r) => 
            {
                if (this.remember)
                {
